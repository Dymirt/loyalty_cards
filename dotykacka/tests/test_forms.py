...
class RegistrationFormTests(TestCase):
    def test_valid_form_normalizes_card_code(self):
        form = LoyaltyCustomerRegistrationForm({**REGISTRATION_DATA, "barcode": " mb-12 "})
        self.assertTrue(form.is_valid(), form.errors)
        self.assertEqual(form.cleaned_data["barcode"], "MB-12")

    def test_rejects_invalid_card_code(self):
        form = LoyaltyCustomerRegistrationForm({**REGISTRATION_DATA, "barcode": "MB-ABC"})
        self.assertFalse(form.is_valid())
        self.assertIn("barcode", form.errors)

    def test_rejects_duplicate_card_code(self):
        create_klient("MB-12")
        form = LoyaltyCustomerRegistrationForm(REGISTRATION_DATA)
        self.assertFalse(form.is_valid())
    ...
    def test_requires_valid_phone_and_consent(self):
        ...
        form = LoyaltyCustomerRegistrationForm(data)
        self.assertFalse(form.is_valid())
    ...
    def test_maps_legacy_post_field_names(self):
        ...
        form = LoyaltyCustomerRegistrationForm(registration_form_data(legacy_data))
        self.assertTrue(form.is_valid(), form.errors)
